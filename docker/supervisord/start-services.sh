...
then
  ...
  then
    cp "/app/templates/$KAVA_NETWORK/app.toml" /data/kava/config/app.toml
  fi

  if [ ! -f "/data/kava/config/config.toml" ]
  then
    cp "/app/templates/$KAVA_NETWORK/config.toml" /data/kava/config/config.toml
  fi
