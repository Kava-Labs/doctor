...
func main() {
	http, err := kava.NewHTTPClient(os.Getenv("KAVA_RPC_URL"))
	...
	go func() {
		for {
			...
			if err != nil {
				// fmt.Printf("error %s getting node status", err)
				time.Sleep(5 * time.Second)
				continue
			}

			// fmt.Printf("%+v synched up to block %d @ %d\n", nodeInfo[0].Metadata, *status.CurrentIndex, time.Now().Unix())
	...
	for {
		...
		case newBlockHeight := <-observedBlockHeights:
			p.Text = fmt.Sprintf("Latest Block Height %d", newBlockHeight)
			draw(tickerCount)
		case <-ticker:
